...
function StringBuilder() {
    this.strings = [], this.length = 0
}
function jdThickBoxclose() {
    $(".thickclose").trigger("click")
}
function getparam() {
    for (var a = "", b = "", c = ({}, location.search.substring(1)), d = c.split("&"), e = 0; e < d.length; e++) {
        var f = d[e].indexOf("=");
        if (-1 != f) {
            {
                d[e].substring(0, f)
...
var TrimPath;
!function() {
    null == TrimPath && (TrimPath = {}), null == TrimPath.evalEx && (TrimPath.evalEx = function(src) {
        return eval(src)
    });
    var UNDEFINED;
        ...
        this.process = function(a, b) {
            ...
            try {
                ...
                if (1 == b.throwExceptions)
                    throw h;
                var i = String(f.join("") + "[ERROR: " + h.toString() + (h.message ? "; " + h.message : "") + "]");
